...
async function tick(): Promise<void> {
  ...
  // Expire overdue payment intents
  type PiRow = { id: string; environment: string };
  const expired = db.prepare("SELECT id,environment FROM payment_intents WHERE status IN ('requires_payment','processing') AND expires_at < ?").all(now) as PiRow[];
